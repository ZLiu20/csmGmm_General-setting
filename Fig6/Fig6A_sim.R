...
# betaMin <- c(0.08 + 0.01 * effSizeMult, 0.08 + 0.01 * effSizeMult, 0.08 + 0.01 * effSizeMult)
# betaMax <- c(0.18 + 0.01 * effSizeMult, 0.18 + 0.01 * effSizeMult, 0.18 + 0.01 * effSizeMult)

# betaMin <- c(0.08 + 0.01 * effSizeMult, 0.08 + 0.01 * effSizeMult, 0.08 + 0.01 * effSizeMult)
# betaMax <- c(0.23 + 0.01 * effSizeMult, 0.23 + 0.01 * effSizeMult, 0.23 + 0.01 * effSizeMult)

betaMin <- c(0.08 + 0.01 * effSizeMult, 0.08 + 0.01 * effSizeMult, 0.08 + 0.01 * effSizeMult)
betaMax <- c(0.28 + 0.01 * effSizeMult, 0.28 + 0.01 * effSizeMult, 0.28 + 0.01 * effSizeMult)
...
for (sim_it in 1:nSims) {
  ...
  #causalVec <- as.numeric( rowSums(allBeta != 0) >= t+1 & rowSums(allBeta != 0) <= nDims & rowSums(allBeta != 0) == abs(apply(sign(allBeta), 1, sum)) )  #revised on 12/11/2025
  causalVec <- as.numeric( 
    abs(apply(sign(allBeta), 1, sum)) >= t+1 &
    abs(apply(sign(allBeta), 1, sum)) <= nDims 
  )  #revised on 12/11/2025
  ...
  if (doNew) {
    ...
    # sameDirAlt=TRUE,newRes <- symm_fit_ind_EM(testStats = allZ, initMuList = initMuList, initPiList = initPiList, sameDirAlt=TRUE, eps=10^(-5))
    newRes <- symm_fit_ind_EM_R1(t_value = t, testStats = allZ, initMuList = initMuList, initPiList = initPiList, sameDirAlt=TRUE, eps=10^(-5))
