...
# setwd("/path/to/csmGmm_reproduce/Fig6/summarize_ukb_analysis.R") or set the path after the -cwd flag
# in the .lsf file, and then run again.
# setwd("~/Downloads/csmGmm_sim_R3/Fig6")
here::i_am("Fig6/summarize_ukb_analysis.R")
...
aID <- as.numeric(args[1])    # aID = 1 for pleiotropy, aID = 2 for replication;
...
fnameRoot <- paste0(outputDir, "/Fig6_data_aID", 1:2)
...
fnameadaFilter <- c(paste0(fnameRoot, "_adaFilter.txt"))
fnameQCH <- c(paste0(fnameRoot, "_qch10.txt"))
...
if (file_it == 1) {
  ...
  tempRes <- data.frame(Method=c("New", "Kernel", "df50", "df7", "qch_copula", "adaFilter"), numReject=NA)
  
  # new 
  tempNew <- fread(fnameNew[file_it], header=T, data.table=F)
  tempDat <- cleanZ %>% select(RS,all_of(selections[[file_it]]), chrpos) %>%
  ...
  # qch_copula
  tempqch <- fread(fnameQCH[file_it], header=T, data.table=F)
  tempDat <- tempDat %>% mutate(qchLfdr = tempqch$x) %>%
    arrange(qchLfdr) %>%
    mutate(cumqch = cummean(qchLfdr)) %>%
    mutate(rejqch = ifelse(cumqch < 0.1, 1, 0)) %>%
    arrange(origIdx)
  tempRes$numReject[4] <- sum(tempDat$rejqch)
  
  # adaFilter
  tempadaFilter <- fread(fnameadaFilter[file_it], header=T, data.table=F)
  tempDat <- tempDat %>% mutate(adaFilterLfdr = tempadaFilter$x) %>%
    arrange(adaFilterLfdr) %>%
    mutate(cumadaFilter = cummean(adaFilterLfdr)) %>%
    mutate(rejadaFilter = ifelse(cumadaFilter < 0.1, 1, 0)) %>%
    arrange(origIdx)
  tempRes$numReject[4] <- sum(tempDat$rejadaFilter)
  
  # any rejection
  tempDat <- tempDat %>% mutate(rejAny = ifelse(rejqch == 1 | rejadaFilter == 1 | 
